...
if lib.xla_extension_version < 57:
  ...
  def _update_all_transfer_guard_global(val):
    config.update('jax_transfer_guard_host_to_device', val)
    config.update('jax_transfer_guard_device_to_device', val)
    config.update('jax_transfer_guard_device_to_host', val)

  def _update_all_transfer_guard_thread_local(val):
    _thread_local_state.jax_transfer_guard_host_to_device = val
    _thread_local_state.jax_transfer_guard_device_to_device = val
    _thread_local_state.jax_transfer_guard_device_to_host = val

    state = transfer_guard_lib.thread_local_state()
    _update_transfer_guard(state, 'host_to_device', val)
    _update_transfer_guard(state, 'device_to_device', val)
    _update_transfer_guard(state, 'device_to_host', val)

  transfer_guard = config.define_enum_state(
      ...
      update_thread_local_hook=_update_all_transfer_guard_thread_local)
